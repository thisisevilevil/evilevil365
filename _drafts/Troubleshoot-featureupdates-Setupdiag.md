...
![Feature Update report](/assets/images/2025-13-01-Troubleshooting-feautreupdate-issues/FeatureUpdate-Reports.png?raw=true "DeviceReadines_BlockingUpgrade_Example.png")
...
$setupdiag = "C:\Windows\temp\setupdiag.exe"
$targetpath = "C:\Windows\Logs\FeatureUpdateIssues"
$args = "/Output:$targetpath\FeatureUpdate_Diagnostics-$env:computername.xml /Format:xml"
if (!(Test-path $setupdiaguri)) {Invoke-Webrequest $setupdiaguri -outfile $setupdiag}
if (!(Test-path $targetpath)) {New-Item $targetpath -ItemType Directory}

#Launch setupdiag with & instead of start-process - Usually works better when the process tends to spawn a new window
& $setupdiag $args
```
...
There is more nuances to troubleshooting feature update failures than you might think, but if a any known signature of a known feature update failure is located by setupdiag, you will be able to clearly see it in the .xml file that it generates next to the Logs.zip file. Here are is a simple real world examples from one of my customers which I have recently assisted:
...
As we can clearly see, the feature update has failed due to lack of disk space. There's plenty of reasons for a feature update to fail, suffice to say, setupdiag will make it easier for you troubleshoot these kind of issues. Using Remediations is one way of triggering setupdiag
